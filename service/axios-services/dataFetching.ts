...
const UpdatePassword = async (phoneNumber: string, password: string) => {
  try {
    const respone = await apiService.patch("api/auth/forgotpassword", {
      phoneNumber: phoneNumber,
      password: password,
    });
    return respone;
...
const ValidatePhoneNumber = async (phoneNumber: string) => {
  try {
    const response = await apiService.post("api/user/validate-phone", {
      phoneNumber: phoneNumber,
    });
    return response;
  } catch (error) {
...
const SignUpValidation = async (phoneNumber: string) => {
  try {
    const response = await apiService.post("api/user/signup-validation", {
      phoneNumber: phoneNumber,
    });
    return response;
  } catch (error) {
...
const UpdateWatchTime = async (userid: string, watchTime: Number) => {
  try {
    const response = await apiService.post("/api/courses/watchtime", {
      userid: userid,
      watchTime: watchTime,
    });
    return;
...
const uploadCsv = async (data: any) => {
  try {
    const response = await apiService.post("/api/admin/bde/upload_csv", {
      csvData: data,
    });
    return response;
  } catch (error) {
...
const verifyAdminMobile = async (mobile: string) => {
  try {
    const response = await apiService.post("/api/admin/bde/verify-mobile", {
      mobile: mobile,
    });
