  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await apiService.get(
        `/api/courses/quiz/?chapterId=${chapterId}`
      );
      ...
      {!toggle ? (
        <>
          <div className="h-full w-full dark">
            <textarea
              className="mb-2 border w-full focus:outline-none pl-3 py-2 h-[100px] rounded-md bg-black"
              value={newQuestion}
            ...
            <div className="flex justify-between mb-4">
              {newOptions.slice(0, 2).map((option, index) => (
                <div key={index} className="flex items-center space-x-2 dark">
                  <input
                    className="me-3 dark"
                    type="radio"
                    name="correctOption"
                  ...
                  />
                  <input
                    className="me-3 border pl-3 rounded-md focus:outline-none dark bg-black"
                    value={option}
                    onChange={(e) => {
            ...
            <div className="flex justify-between mb-4">
              {newOptions.slice(2).map((option, index) => (
                <div key={index + 2} className="flex items-center space-x-2">
                  ...
                  />
                  <input
                    className="me-3 border pl-3 rounded-md focus:outline-none bg-black"
                    value={option}
                    onChange={(e) => {
                      const updatedOptions = [...newOptions];
            ...
            <div className="flex flex-col items-center justify-center h-full w-full">
              <button
                className="bg-green-400 text-white w-[300px] rounded-lg hover:bg-green-600 mb-3 h-[30px]"
                onClick={handleAddQuestion}
              >
                Add Question
              </button>
              {publishedQuestion.length > 0 && (
                <h1
                  className="text-sm text-blue-500 rounded-lg mb-5 cursor-pointer mx-3"
                  onClick={viewPublishedQuestions}
                >
                      ...
                      {question?.options?.length > 1 &&
                        question.options
                          ...
                          .map((option: any, index) => (
                            <div
                              className="flex justify-between my-2 w-[300px]"
                              key={option.id}
                            >
            ...
            {questions.length > 0 ? (
              <div className="flex flex-col items-center justify-center mt-8 h-full w-full">
                <button
                  className="bg-orange-400 text-white w-[300px] rounded-lg hover:bg-orange-600 mb-5 h-[30px]"
        ...
        <div className="flex flex-col dark">
          {publishedQuestion.length > 0 &&
            publishedQuestion.map((question: any, index) => (
              <div key={index}>
